...
Fast path: the same `bash skill/c-cpp-profi/scripts/cpp_comprehension_map.sh <repo>` emits the L2 **exported-API list** (non-static function declarations in public headers — the real entry points of a library, e.g. inih `ini_parse`, logc `log_*`, each at `file:line`), the L2 **entry-point list** (`main(`, `LLVMFuzzerTestOneInput`, exported-symbol hints, and public headers, each at a repo-relative `file:line` or path; a `main()` behind a `#if <NAME>_MAIN` self-test guard is labeled a *conditional test driver*, and doc-comment `main()` in headers is not counted), and a coarse **module map** (top-level source dirs with per-dir file counts). The exported-API extractor handles the macro/paren-wrapped declaration idioms serious C libraries use — a paren-wrapped name (`LUA_API int (lua_absindex)(...)` → `lua_absindex`), a leading export-macro prefix (`UV_EXTERN`/`*_API`/`*_EXPORT`/`*_PUBLIC`), a macro-wrapped return type (`CJSON_PUBLIC(cJSON *) cJSON_Parse(...)`), and a multi-line decl with a trailing macro suffix (`BaseType_t xTaskCreate(...) PRIVILEGED_FUNCTION;`) — while excluding non-functions (`MBEDTLS_PRIVATE(field)` struct-field markers, reserved/asm `__…__` tokens, ALL-CAPS macro-shaped names); C++ `static` member functions in public headers (e.g. `DB::Open`) are kept. The long symbol-hint / exported-API lists are deduped and capped with a `... (+N more; capped)` footer; the exported-API cap ranks likely-public decls first (those under top-level `include/**` or carrying an export macro) and round-robins across source files so no single header monopolizes the cap and internal/per-arch symbols never bury the public surface. Caveat: it is a heuristic, not a C front-end — within a large API a specific late-alphabet headline symbol can still fall past the cap (read the full list, or `nm -D`, when you need an exact name). The same probe now also emits an L3 **touched-path callgraph**: from a few seed entry points (`main`, `LLVMFuzzerTestOneInput`, and leading exported-API symbols) it draws `caller -> callee` edges restricted to functions *defined in this repo*, two levels deep, deduped and capped — verified to recover the real parse path (cJSON `cJSON_Parse -> cJSON_ParseWithOpts`; inih `ini_parse -> ini_parse_file`, and C++ method dispatch `INIReader::GetBoolean -> Get -> MakeKey`). It is a token-scan heuristic, not a compiler callgraph: self-recursion IS surfaced (a self-calling function is shown as `name -> name (recursive)`, e.g. cJSON's `cJSON_Delete`), but function-pointer/vtable calls (e.g. cJSON's `global_hooks.deallocate`), overloaded names, and macro-generated calls are still missed, and a function whose only callee is one of those is honestly printed as `(no in-repo callees)`.

For an **exact** graph, pass `--exact`: `cpp_comprehension_map.sh --exact <repo>` lowers each TU that compiles standalone to LLVM IR (`clang -O0 -g -emit-llvm -S`, best-effort include flags) and reads `call`/`invoke @symbol` edges straight from the IR, so there are no token false positives and the edges are the compiler's own (verified on cJSON: `cJSON_Parse -> cJSON_ParseWithOpts`, `cJSON_Minify -> minify_string, skip_*_comment`; and on C++ tinyxml2 with `c++filt` demangling). It is off by default because it is the one mode that compiles code; it degrades to a one-line note when clang is absent or no TU compiles standalone (then the heuristic graph stands), and a repo that needs a real build yields fewer compiled TUs (point it at one with `compile_commands.json` for full coverage). Even the exact graph omits **indirect** calls (function pointers, C++ virtual dispatch): those carry no symbol in the IR and are not resolvable by any purely static analysis — for those, run the code under a dynamic call tracer (`perf`, `callgrind --dump-instr`, or a ubsan/asan-instrumented run). `cscope -bR` / clangd `callHierarchy` remain alternative exact tools when installed.
