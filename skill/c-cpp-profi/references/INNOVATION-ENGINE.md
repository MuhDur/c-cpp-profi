...
> **Tooling status (loop-tracked).** `cpp_inventory.sh`, `cpp_risk_scan.sh`, and `cpp_evidence_check.py`
> (profiles `basic/parser/memory/public-abi/concurrency/performance/refactor/native-ui/portability/security/docs-scripts`)
> exist today. The proposal-specific tooling named below — `cpp_backlog.sh`, `cpp_idea_check.py`, and the
> `comprehension`/`idea`/`port`/`modernize`/`rearchitect` profiles plus `--derive-profiles`/`--strict-numeric` —
